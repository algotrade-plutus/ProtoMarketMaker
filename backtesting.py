...
class Backtesting:
    ...
    def __init__(
        ...
        # bid-ask spread
        self.bid_spread = Decimal("0.0")
        self.ask_spread = Decimal("0.0")
    ...
    def move_f1_to_f2(self, f1_price, f2_price):
        ...
        if self.inventory["BUY"] != 0:
            ...
            self.ac_loss += Decimal('0.47') * Decimal('100') * self.inventory["BUY"]
        elif self.inventory["SELL"] != 0:
            self.ac_loss += (f1_price - self.inventory_price["SELL"]) * 100
            self.inventory_price["SELL"] = f2_price
            self.ac_loss += Decimal('0.47') * Decimal('100') * self.inventory["SELL"]
    ...
    def handle_force_sell(self, price: Decimal):
        ...
        while placeable_sell < 0:
            ...
            self.ac_loss += (price - self.inventory_price["SELL"]) * 100 + Decimal(
                '0.47'
        ...
        while placeable_buy < 0:
            ...
            self.ac_loss += (self.inventory_price["BUY"] - price) * Decimal(
                '100'
            ) + Decimal('0.47') * Decimal('100') / Decimal('2')
    ...
    def handle_matched_order(self, price):
        ...
        if self.bid_price >= price and self.inventory["BUY"] < placeable_buy:
            ...
            self.ac_loss += Decimal('0.47') * Decimal('100') / Decimal('2')
            self.daily_buy += 1
    ...
    def update_bid_ask(self, price: Decimal, step):
        """
        ...
        self.bid_price = price - step * (self.inventory["BUY"] + 1)
        self.ask_price = price + step * (self.inventory["SELL"] + 1)
    ...
    def run(self, data: pd.DataFrame, step: Decimal):
        ...
        for index, row in data.iterrows():
            ...
            self.update_bid_ask(row["f2_price"] if moving_to_f2 else row["price"], step)
